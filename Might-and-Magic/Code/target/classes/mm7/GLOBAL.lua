...
evt.CanShowTopic[18] = function()
	...
	if not evt.Cmp("QBits", 7) then         -- Finished Scavenger Hunt
		result = not evt.Cmp("Inventory", 145)         -- "Alacorn Wand of Fireballs"
	end
	return false
end
...
evt.global[19] = function()
	...
	evt.Set("AutonotesBits", 1)         -- "Accepted fireball wand from Mr. Malwick on Emerald Island."
...
evt.global[125] = function()
	if evt.Cmp("Counter1", 672) then
		...
		if not evt.Cmp("NPCs", 71) then         -- "Loren Steel"
			if evt.Cmp("NPCs", 72) then         -- "Fake Loren"
...
evt.CanShowTopic[127] = function()
	...
	if not evt.Cmp("QBits", 92) then         -- Fort Riverstride.  Opened chest with plans
		result = not evt.Cmp("Counter2", 672)
	end
	return false
end
...
evt.global[128] = function()
	if evt.Cmp("QBits", 82) then         -- Gave false plans to elfking (betray)
		...
		if evt.Cmp("QBits", 91) then         -- Told Elfking about fake plans
...
evt.global[132] = function()
	if evt.Cmp("Counter2", 672) then
		...
		evt.ForPlayer("All")
		if not evt.Cmp("Inventory", 705) then         -- "Riverstride Plans"
			if evt.Cmp("Inventory", 706) then         -- "False Riverstride Plans"
...
evt.CanShowTopic[134] = function()
	...
	if not evt.Cmp("QBits", 93) then         -- Tularean Caves.  Got Loren
		result = not evt.Cmp("Counter1", 672)
	end
	return false
end
...
evt.global[135] = function()
	if evt.Cmp("QBits", 83) then         -- Gave false Loren to Catherine (betray)
		...
		if evt.Cmp("QBits", 90) then         -- Told Catherine about fake prisoner
...
evt.global[317] = function()
	...
	evt.Set("AutonotesBits", 130)         -- "The Master instructor of the Sword, Tugor Slicer, maintains his residence in a small village in the western region of Deyja."
end
...
evt.global[337] = function()
	...
	evt.Set("AutonotesBits", 150)         -- "The Master tutor of Fire Magic, Ashen Temper, can be found in the city of Harmondale near the stables."
end
...
evt.global[338] = function()
	...
	evt.Set("AutonotesBits", 151)         -- "The Grandmaster of Fire Magic, Blayze, makes his home in Erathia in the capitol city of Steadwick."
end
...
evt.global[341] = function()
	...
	evt.Set("AutonotesBits", 154)         -- "The Master tutor of the uses of Water Magic, Tobren Rainshield, lives in the southwest region of Nighon."
end
...
evt.CanShowTopic[422] = function()
	...
	if not evt.Cmp("Awards", 14) then         -- "Promoted to Assassin"
		if not evt.Cmp("Awards", 15) then         -- "Promoted to Honorary Assassin"
			result = evt.Cmp("QBits", 21)         -- "Go to the Celestial Court in Celeste and kill Lady Eleanor Carmine.  Return with proof to Seknit Undershadow in the Deyja Moors."
		end
	end
	return true
end
...
evt.CanShowTopic[424] = function()
	...
	if not evt.Cmp("Awards", 40) then         -- "Promoted to Crusader"
		if not evt.Cmp("Awards", 41) then         -- "Promoted to Honorary Crusader"
			result = evt.Cmp("QBits", 22)         -- "Kill Wromthrax the Heartless in his cave in Tatalia, then talk to Sir Charles Quixote."
		end
	end
	return true
end
...
evt.CanShowTopic[515] = function()
	...
	if not evt.Cmp("QBits", 252) then         -- Bought Item 3
		result = not evt.Cmp("QBits", 256)         -- Can't keep buying 1
	end
	return true
end
...
evt.global[516] = function()
	...
	if evt.Cmp("Inventory", 694) then         -- "Crate of Tularean Wood"
		...
		if evt.Cmp("DayOfWeekIs", 0) then
			...
			if evt.Cmp("DayOfWeekIs", 1) then
				evt.Add("Gold", 241)
			else
				if evt.Cmp("DayOfWeekIs", 2) then
					evt.Add("Gold", 251)
				else
					if evt.Cmp("DayOfWeekIs", 3) then
						evt.Add("Gold", 237)
					else
						if evt.Cmp("DayOfWeekIs", 4) then
							evt.Add("Gold", 273)
						else
							if evt.Cmp("DayOfWeekIs", 5) then
								evt.Add("Gold", 255)
							else
								if evt.Cmp("DayOfWeekIs", 6) then
									evt.Add("Gold", 286)
								end
							end
						end
...
evt.CanShowTopic[517] = function()
	...
	if not evt.Cmp("QBits", 250) then         -- Bought Item 1
		result = not evt.Cmp("QBits", 257)         -- Can't keep buying 2
	end
	return true
end
...
evt.CanShowTopic[519] = function()
	...
	if not evt.Cmp("QBits", 251) then         -- Bought Item 2
		result = not evt.Cmp("QBits", 258)         -- Can't keep buying 3
	end
	return true
end
...
evt.CanShowTopic[521] = function()
	...
	if not evt.Cmp("QBits", 255) then         -- Bought Item 6
		result = not evt.Cmp("QBits", 259)         -- Can't keep buying 4
	end
	return true
end
...
evt.global[522] = function()
	...
	if evt.Cmp("Inventory", 697) then         -- "Crate of Enrothian Wine"
		...
		if not evt.Cmp("Counter9", 24) then
			...
			if not evt.Cmp("Counter9", 120) then
				evt.Add("Gold", 2750)
			else
				if evt.Cmp("Counter9", 240) then
					evt.Add("Gold", 2500)
				else
					evt.Add("Gold", 2250)
				end
...
evt.CanShowTopic[523] = function()
	...
	if not evt.Cmp("QBits", 253) then         -- Bought Item 4
		result = not evt.Cmp("QBits", 260)         -- Can't keep buying 5
	end
	return true
end
...
evt.global[524] = function()
	...
	if evt.Cmp("Inventory", 695) then         -- "Crate of Sand"
		...
		if not evt.Cmp("Counter7", 24) then
			...
			if not evt.Cmp("Counter7", 120) then
				evt.Add("Gold", 2750)
			else
				if evt.Cmp("Counter7", 240) then
					evt.Add("Gold", 2500)
				else
					evt.Add("Gold", 2250)
				end
...
evt.CanShowTopic[525] = function()
	...
	if not evt.Cmp("QBits", 254) then         -- Bought Item 5
		result = not evt.Cmp("QBits", 261)         -- Can't keep buying 6
	end
	return true
end
...
evt.global[526] = function()
	...
	if evt.Cmp("Inventory", 696) then         -- "Crate of Glass Bottles"
		...
		if not evt.Cmp("Counter8", 24) then
			...
			if not evt.Cmp("Counter8", 120) then
				evt.Add("Gold", 2750)
			else
				if evt.Cmp("Counter8", 240) then
					evt.Add("Gold", 2500)
				else
					evt.Add("Gold", 2250)
				end

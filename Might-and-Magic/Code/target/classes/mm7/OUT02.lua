...
evt.map[211] = function()  -- function events.LoadMap()
	if not evt.Cmp("QBits", 134) then         -- Arbiter Messenger only happens once
		if evt.Cmp("Counter3", 2688) then
			...
			evt.MoveNPC{NPC = 76, HouseId = 174}         -- "Ambassador Scale" -> "Throne Room"
...
evt.hint[218] = evt.str[4]  -- "Drink from the Well"
...
evt.map[221] = function()
	if evt.Cmp("QBits", 246) then         -- Visited The Land of the giants
		evt.MoveToMap{X = 4221, Y = 17840, Z = 769, Direction = 1536, LookAngle = 0, SpeedZ = 0, HouseId = 0, Icon = 0, Name = "out12.odm"}
	else
...
evt.house[256] = 470  -- "Temper Residence"
evt.map[256] = function()
	if evt.Cmp("QBits", 183) then         -- Failed either goto or do guild quest
		evt.StatusText(21)         -- "This Door is Locked"
		evt.FaceAnimation{Player = "Current", Animation = 18}
	else
		evt.EnterHouse(470)         -- "Temper Residence"
	end
end
...
evt.house[263] = 490  -- "Hume Residence"
evt.map[263] = function()
	if evt.Cmp("QBits", 183) then         -- Failed either goto or do guild quest
		evt.StatusText(21)         -- "This Door is Locked"
		evt.FaceAnimation{Player = "Current", Animation = 18}
	else
		evt.EnterHouse(490)         -- "Hume Residence"
	end
end

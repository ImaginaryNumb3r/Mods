...
local function LoadUI(i)
	...
	for k,v in pairs(Game.UISets) do
		if v.LoadedTo > 0 then
			FreeCustomLod(v.LoadedTo)
			v.LoadedTo = 0
		end
		if v.DLoadedTo > 0 then
			FreeCustomLod(v.DLoadedTo)
			v.DLoadedTo = 0
		end
	end

	if New.LodName ~= "default" then
		New.LoadedTo = LoadCustomLod(Game.IconsLod["?ptr"], FilesPath .. New.LodName)
	end
	if New.DLodName ~= "" then
		New.DLoadedTo = LoadCustomLod(DLODPTR, FilesPath .. New.DLodName)
	end

...
function events.GameInitialized2()
	...
	OpenFlwPnlBtn = CustomUI.CreateButton{
							...
							Condition = function() return NPCPanelOpen or (mem.u4[InGame] == 1 and vars and vars.NPCFollowers and table.maxn(vars.NPCFollowers) > 0) end,
							Layer = 2,
							Screen = 0}

	function events.FGInterfaceUpd()
		if StdCond() and Game.CurrentScreen == 0 then
			for i = 1, 3 do
	...
	function events.Action(t)
		if Game.CurrentScreen == 0 and StdCond() and CustomUI.MouseInBox(FollowersBG.X, FollowersBG.Y, FollowersBG.Wt, FollowersBG.Ht) then
			t.Handled = true

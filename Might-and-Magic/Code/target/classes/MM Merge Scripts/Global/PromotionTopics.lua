--------------------------------------------
---- Learn basic blaster (Tolberti, Rober the Wise)
...
local function SetLichAppearance()
	for i,v in Party do
		if v.Class == 45 then
			local CurSex = Game.CharacterPortraits[v.Face].DefSex
			v.Face = 26 + CurSex
			v.Voice = 26 + CurSex
			SetCharFace(i, 26 + CurSex)
			for i = 0, 3 do
...
evt.Global[738] = PromoteDarkMages
...
function events.EnterNPC(i)
	...
	if evt.ForPlayer("All").Cmp{"ClassIs", 48} then
		...
		for i = 0, 4 do
			if cNPC.Events[i] == 0 or cNPC.Events[i] == PeasantPromoteTopic then
				cEvent = i
				break
			end
		end
...
evt.CanShowTopic[PeasantPromoteTopic] = function()
	return evt.ForPlayer("All").Cmp{"ClassIs", 48}
end

...
local function ControlTimer()
	local Pl
	for k,v in pairs(PSet.Buffs) do

		-- Divine Transcendence
		if v.DivTransActive and Game.Time > v.DivTransExpire then
			Pl = Party.PlayersArray[k]
			Pl.LevelBonus = Pl.LevelBonus - v.DivTransPower
			v.DivTransActive = false
		end
...
--	Timer(ControlTimer, const.Minute*5)
--end
...
evt.PotionEffects[62] = function(IsDrunk, Target, Power)
	if IsDrunk then
		...
		--PSet.Buffs[PlayerId] = PSet.Buffs[PlayerId] or {}
		--local t = PSet.Buffs[PlayerId]
		--t.DivTransActive = true
		--t.DivTransPower = t.DivTransPower or 0
		--t.DivTransPower = t.DivTransPower + 20
		--t.DivTransExpire = Game.Time + Power*const.Minute*30
...
evt.PotionEffects[70] = function(IsDrunk, Target, Power)
	if IsDrunk then
		...
		Buff.ExpireTime = Game.Time + const.Minute*30*Power
		Buff.Power = 3
		Buff.Skill = 7
	end
end

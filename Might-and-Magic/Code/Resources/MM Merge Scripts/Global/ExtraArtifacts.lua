...
function events.GetAttackDelay(t)
	...
	if PLT then
		...
		local MHItem = Pl.ItemMainHand
		if MHItem > 0 and Game.ItemsTxt[Pl.Items[MHItem].Number].Skill == 7 then
...
WearItemConditions[504] = function(PlayerId)
	return Game.CharacterPortraits[Party[PlayerId].Face].Race == const.Race.Minotaur
end

-- Foulfang
WearItemConditions[508] = function(PlayerId)
	return Game.CharacterPortraits[Party[PlayerId].Face].Race == const.Race.Vampire
end
...
WearItemConditions[1406] = function(PlayerId)
	local EqItems = Party[PlayerId].EquippedItems
	...
	local MIt = EqItems[1]
	if MIt > 0 then
		local Item = Game.ItemsTxt[Party[PlayerId].Items[MIt].Number]

...
evt.map[2] = function()  -- function events.LeaveMap()
	if not evt.CheckMonstersKilled{CheckType = 1, Id = 5, Count = 1} then
		if not evt.CheckMonstersKilled{CheckType = 1, Id = 4, Count = 1} then
			return
		end
	end
	evt.Set("MapVar4", 2)
...
evt.house[376] = 522  -- "Alice Hargreaves"
evt.map[376] = function()
	if not evt.Cmp("QBits", 25) then         -- Mini-dungeon Area 5.  Rescued/Captured Alice Hargreaves.
		if not evt.Cmp("NPCs", 54) then         -- "Alice Hargreaves"
			if evt.Cmp("QBits", 26) then         -- "Capture Alice Hargreaves from her residence in Castle Gryphonheart and return her to William's Tower in the Deyja Moors."
				evt.EnterHouse(522)         -- "Alice Hargreaves"
				return
			end
		end
	end
...
evt.map[377] = function()
	if not evt.Cmp("QBits", 264) then         -- Took Roland Painting
		evt.SetTexture{Facet = 15, Name = "chb1"}
		evt.Add("Inventory", 623)         -- "Roland Ironfist Painting"
		evt.Add("QBits", 264)         -- Took Roland Painting
	end
end

evt.hint[378] = evt.str[15]  -- "Portrait"
evt.map[378] = function()
	if not evt.Cmp("QBits", 265) then         -- Took Archi Painting
		evt.SetTexture{Facet = 16, Name = "chb1"}
		evt.Add("Inventory", 622)         -- "Archibald Ironfist Painting"
		evt.Add("QBits", 265)         -- Took Archi Painting
	end
end

evt.house[416] = 176  -- "Throne Room"
evt.map[416] = function()
	if evt.Cmp("QBits", 98) then         -- Built Castle to Level 2 (rescued dwarf guy)
		if not evt.Cmp("MapVar4", 2) then
			evt.EnterHouse(176)         -- "Throne Room"
			return
		end
	end
	evt.FaceAnimation{Player = "Current", Animation = 18}
...
evt.map[501] = function()
	evt.MoveToMap{X = -486, Y = 9984, Z = 2401, Direction = 1024, LookAngle = 0, SpeedZ = 0, HouseId = 0, Icon = 8, Name = "Out03.odm"}
end

evt.hint[502] = evt.str[2]  -- "Leave Castle Gryphonheart"
evt.map[502] = function()
	if evt.Cmp("Inventory", 660) then         -- "Catherine's Key"
		evt.MoveToMap{X = 1050, Y = 9991, Z = 2913, Direction = 1024, LookAngle = 0, SpeedZ = 0, HouseId = 0, Icon = 8, Name = "Out03.odm"}
	else
		evt.StatusText(21)         -- "The Door is Locked"
		evt.FaceAnimation{Player = "Current", Animation = 18}
	end

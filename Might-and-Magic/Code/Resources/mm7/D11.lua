...
evt.map[176] = function()
	if evt.Cmp("QBits", 48) then         -- "Retrieve the lich jars from the Proving Grounds in Celeste and bring them back to Halfgild Wynac in the Pit."
		evt.OpenChest(0)
...
evt.map[466] = function()
	if not evt.Cmp("MapVar11", 1) then
		...
		if not evt.Cmp("MapVar10", 1) then
			...
			if not evt.Cmp("MapVar10", 2) then
				evt.SetDoorState{Id = 25, State = 2}         -- switch state
				evt.Add("MapVar10", 1)
			else
				if evt.Cmp("MapVar10", 3) then
